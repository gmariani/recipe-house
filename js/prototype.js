...
Ajax.Request.prototype = Object.extend(new Ajax.Base(), {
    ...
    request: function (url) {
        ...
        try {
            this.url = url;
            if (this.options.method == 'get' && parameters.length > 0) this.url += (this.url.match(/\?/) ? '&' : '?') + parameters;
...
Element.extend = function (element) {
    ...
    if (!element._extended && element.tagName && element != window) {
        ...
        for (var property in methods) {
            ...
            if (typeof value == 'function') element[property] = cache.findOrStore(value);
        }
    }
...
Selector.prototype = {
    ...
    buildMatchExpression: function () {
        ...
        if ((clause = params.id)) conditions.push('element.id == ' + clause.inspect());
        if ((clause = params.tagName)) conditions.push('element.tagName.toUpperCase() == ' + clause.inspect());
        if ((clause = params.classNames).length > 0) for (var i = 0; i < clause.length; i++) conditions.push('Element.hasClassName(element, ' + clause[i].inspect() + ')');
    ...
    findElements: function (scope) {
        ...
        var results = [];
        for (var i = 0; i < scope.length; i++) if (this.match((element = scope[i]))) results.push(Element.extend(element));
...
Abstract.EventObserver.prototype = {
    ...
    registerFormCallbacks: function () {
        ...
        for (var i = 0; i < elements.length; i++) this.registerCallback(elements[i]);
    },
